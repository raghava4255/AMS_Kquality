...
export const Dashboard = () => {
  ...
  useEffect(() => {
    if (empTab === 'leaves') fetchMyLeaves();
  }, [empTab, fetchMyLeaves]);
        ...
        <div style={styles.dashboardGrid}>
          <div style={styles.leftCol}>
            ...
            <div className="glass-panel" style={styles.quickActionCard}>
              ...
              <p style={styles.quickActionDesc}>Provision new employee accounts with custom passwords.</p>
              <button
                id="add-employee-btn"
          ...
          <div style={styles.rightCol}>
            <div className="glass-panel" style={styles.deptCard}>
              ...
              <div style={styles.deptList}>
                {(user.stats?.departmentStats ?? []).map((dept, i) => (
            ...
            <form onSubmit={handleAddEmployee} style={styles.modalForm}>
              ...
              <div style={styles.modalInputGroup}>
                ...
                <select value={newEmployeeDept} onChange={e => setNewEmployeeDept(e.target.value)} style={styles.modalSelect}>
                  {['Engineering', 'Marketing', 'Sales', 'HR & Ops'].map(d => <option style={{ color: 'black' }} key={d} value={d}>{d}</option>)}
                </select>
              </div>
...
const styles = {
  ...
  navbar:    { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 32px', background: 'rgba(15,18,26,0.45)', borderBottom: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)', zIndex: 50 },
